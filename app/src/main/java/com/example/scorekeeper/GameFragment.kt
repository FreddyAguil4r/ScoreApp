...
class GameFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        //Get data passed from pervious frame
    ...
    private fun team2Shoot(pointValue: Int) {
        if (pointValue != 0) {
            ...
            when {
                binding.gameFragmentRbQ1.isChecked -> {
                    val quarterScore =
                        binding.gameFragmentTvTeam2Q1.text.toString().toInt() + pointValue
                    binding.gameFragmentTvTeam2Q1.text = quarterScore.toString()
                }

                binding.gameFragmentRbQ2.isChecked -> {
                    val quarterScore =
                        binding.gameFragmentTvTeam2Q2.text.toString().toInt() + pointValue
                    binding.gameFragmentTvTeam2Q2.text = quarterScore.toString()
                }

                binding.gameFragmentRbQ3.isChecked -> {
                    val quarterScore =
                        binding.gameFragmentTvTeam2Q3.text.toString().toInt() + pointValue
                    binding.gameFragmentTvTeam2Q3.text = quarterScore.toString()
                }
                else -> {
                    val quarterScore =
                        binding.gameFragmentTvTeam2Q4.text.toString().toInt() + pointValue
